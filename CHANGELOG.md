...
- Recorded item `timestamp` (relative date, e.g. "2 hours ago") in `details["timestamp"]` for `news_quotes`, `twitter_result`, `view_more_news`, and `videos`. These parsers already extracted a timestamp but wrote it to a top-level key the result round-trip silently dropped; it now rides in `details` (only when present, alongside the tweet payload for `twitter_result`)
