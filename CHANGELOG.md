...
- Added a `visible` boolean to every parsed result row (and to carousel `details.items`): `False` when the item lives under an inline `display:none` container (a lazy-loaded carousel tail or "show more" expansion that ships in the static HTML but isn't rendered on initial load), `True` otherwise. Defaults `True` everywhere, so existing consumers are unaffected. Set on the `top_stories` family (`top_stories`, `perspectives`, `local_news`, `recent_posts`, `latest_from`) and the carousels (`videos`, `short_videos`, `top_image_carousel`, `img_cards`, `shopping_ads`, `available_on`); backed by a new `is_hidden` helper
