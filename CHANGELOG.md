...
- Classify the residual `unknown` families left after reparsing three directives crawls at 0.11.1a3 (the prior cross-wave batch already resolved ~89% of the stale-0.11.0a2 survey). Each fix is additive/end-of-chain or a header-text entry, so corpus snapshots are unchanged; across the sampled SERPs the residual unknown components dropped 18 -> 7 (c01), 12 -> 3 (c19), 0 (c20), with the remainder now confined to irreducible states (`Math Solver` JS widgets, dynamic `StandardElection` date displays, per-hotel check-in/entity headers) and a one-off tail. New/extended types and rules:
  - **`gallery`** (new type): the JS-hydrated discovery gallery ("What to read" books, "Courses", "Explore stocks", ...). Classified structurally, end-of-chain, by its `Supercat*ClusterTitle` `data-attrid` (reused across content types -- a *books* cluster carries `SupercatRecipeClusterTitle`) rather than the varying heading. The module ships a hidden `display:none` "Something went wrong / TRY AGAIN" async-error fallback whose text (not shown on the page) previously sorted ahead of the real heading and buried it in the component `text`, so it reached unknown. The parser emits a `sub_type="header"` row (cluster heading as `title`, category-filter chips joined into `text`) followed by one row per item (title + author byline; JS-driven cards, no url; the `display:none` "More items" tail is flagged `visible=False`)
...
- Classify a further batch of cross-wave `unknown` component families surfaced by a six-crawl directives-corpus survey (one representative dir per collection wave). Each fix is additive/end-of-chain or a header-text/`_BAD_LABELS` entry, so corpus snapshots are unchanged; on the 44 sampled SERPs spanning all families the residual unknown components dropped 23 -> 7 (the tail: sponsored "People also consider" chips, failed-load "Something went wrong" states, and per-hotel check-in pickers). New/extended types and rules:
  ...
  - **`articles`** (new type): the entity-panel "Articles" module (external article links, deduped by url, feedback `#` skipped)
  - **`images`** left-bar/inline thumbnail strip: an offscreen a11y `h3.bNg8Rb` "Images" label with no imagebox signal now classifies end-of-chain and parses via a new `sub_type="strip"` fallback (caption anchor + thumbnail anchor deduped by url)
  - **`recipes`** async "Popular recipes" / "More popular recipes" carousel (aria-level-2 heading, often wrapped in "Preferences saved ... RETRY" state chrome): registered at level 2, and `parse_recipes` gained a `div.XDOVcf` card fallback (title div outside the result anchor)
  - **`products`** tray carousel: "Popular products" / "More products" title in an aria-level-2 `g-tray-header` span registered at level 2 (JS-hydrated cards carry no static content, so these type as `products` with the standard "no subcomponents parsed" marker instead of `unknown`)
  - **`knowledge`** submodules: `knowledge_submodule` now scans aria-level 3 (for "Menu highlights") as well as 2, and its heading set gains "Rooms at <hotel>" room-rate modules plus entity-submodule tails ("Interactive diagrams", "Featured events", "How to solve your problem" / "Your math problem", "Explore more", "Top sights in", "Beach/Popular/Surfing/Camping destinations")
  - **Bare a11y section-heading suppression:** `_BAD_LABELS` gains "Web results", "Ads", and "Description" so a bodyless offscreen `h*.bNg8Rb` heading (the empty "Ads" ad-slot shells, "Web results" labels) is dropped at extraction rather than surfacing as an empty `unknown` row; the 15-char scan bound keeps any real section that merely starts with the label
...
- Classify the remaining crawl-6 unknown-component families (second pass after the AI-overview banner below; together they cut a 2026-07 crawl sample's unknown components from 934 to 14):
  ...
  - Hotel carousels: fixed the `locations` classifier's heading match to space-join multi-span headings ("More"+"Hotels" previously read "MoreHotels" and never matched), added an end-of-chain `hotel_carousel` rule for card carousels ("Similar to <hotel>", "Popular hotels in <place>", brand variants) keyed on `local/places/hotel/` anchors or `/travel/search` + hotel-name divs, and added a `parse_hotels` branch for the JS-hydrated card layout (name span + per-card places anchor + price/rating/reviews)
  - Knowledge-panel submodules splatted into the main column now classify as `knowledge` via a new end-of-chain `knowledge_submodule` rule: entity-attribute modules (`lab/title/`/`lab/content/` attrids -- Movies, Played by, Calories, ...), core-answer breadcrumbs (`CoreAnswerModuleHeader`), sports modules (`TLOsrp*` attrids, `sp-table` squad tables), and a small heading tuple with no structural signal ("Menu highlights", "Things to do", "Showtimes at")
  - Flight-status widgets whose heading is a bare `<h2>` (no `role="heading"`) now classify as `flights` and title from the h2
  - End-of-chain placement means the new structural rules only claim otherwise-unknown components; corpus sweeps confirm the only same-row changes are upgrades (hollow `general`/`knowledge` shells becoming typed rows with content) with zero urls or titles lost. Nine curated fixture SERPs pin the new paths
- Capture modern ad sitelinks: a third `parse_ad_menu` format keys on bare `a.ynAwRc` anchors (no `MhgNwc`/`bOeY0b` wrapper), so ads carrying them now parse as `ad/submenu` with a `menu` details payload instead of `ad/standard` with the sitelinks silently dropped. Demo-corpus sweep: 18 ad rows across 17 SERPs flip `standard -> submenu`, no other changes
- Capture the "Sponsored results" ad unit embedded in the "Find related products & services" module: the two render as one DOM block, so `parse_searches_related` now delegates embedded `uEierd` ads to the ads parser and emits them as `ad` rows ahead of the suggestions row (same component, sequential sub_ranks). Demo-corpus sweep: 22 previously invisible ads across 11 SERPs gain rows, no other changes
- Classify the standalone AI-overview loading/unavailable banner (serialized mid-generation as a "Thinking" heading, or "An AI Overview is not available for this search"), previously `unknown`: a new end-of-chain classifier keys on the AI overview's `EYwa3d` controller and routes to `ai_overview` with the existing `sub_type="unavailable"`. The same controller also lives inside components other classifiers own (knowledge panels embedding an AI-overview module, `Fzsovc`-marked overviews), so the rule only claims components nothing earlier in the chain typed -- zero reclassification of previously-typed components across the fixture and demo corpora. On a 2026-07 crawl sample this banner was ~87% of unknown-component volume. Two curated fixture SERPs (one per markup variant) pin the behavior

## [0.11.0] - 2026-07-04

- **Breaking:** removed the `selenium` (undetected-chromedriver), `zendriver`, and `playwright` browser backends, leaving `patchright` (now the **default**) and `requests` (the no-browser HTTP path). Constructing `SearchEngine(method="selenium"|"zendriver"|"playwright")` now raises (the `SearchMethod` enum no longer defines them), the default backend changes from `selenium` to `patchright` (a bare `ws.SearchEngine()` launches patchright instead of undetected-chromedriver), and `SeleniumConfig`/`ZendriverConfig` and the `selenium_config`/`zendriver_config`/`playwright_config` kwargs are gone. **Migration:** pass `method="patchright"` (now the default); the default `channel="chrome"` drives an installed Google Chrome (`patchright install chrome` if missing), or run `patchright install chromium` and set `channel="chromium"` for the bundled browser -- either way the browser binary is a post-install step pip can't run automatically. The install footprint drops `selenium` + `undetected-chromedriver` and adds `patchright` (which bundles its node driver); `requests` is unchanged. This is plan 039's green-lit migration follow-up, now that plan 049 confirmed patchright runs headed on a no-display host via Xvfb (plan 051)
...
- **Breaking (logging):** crawl logs are now emitted as JSON Lines only -- one structured JSON object per line. The text formatters (`minimal`/`medium`/`detailed`) and the `console_format`/`file_format` selectors are gone from both `Logger` and `LogConfig`, so every console and file sink emits the `jsonl` schema. Each line always carries `timestamp`/`pid`/`level` and adds only the keys that apply (`event`; `message`; `response_code`/`qry`/`loc` on search lines; `output` for tracebacks; `source` with `event="external"` for third-party logs), so consumers must read the optional keys with `.get(...)`. This retires the after-the-fact text-log parser for new crawls (plan 052)
- Added `SearchEngine.close()` for deterministic teardown of the searcher backend (closes the browser window / HTTP session), and made `SearchEngine` a context manager (`with ws.SearchEngine() as se:`) that closes on exit
