...
- Classify the residual `unknown` families left after reparsing three directives crawls at 0.11.1a3 (the prior cross-wave batch already resolved ~89% of the stale-0.11.0a2 survey). Each fix is additive/end-of-chain or a header-text entry, so corpus snapshots are unchanged; across the sampled SERPs the residual unknown components dropped 18 -> 7 (c01), 12 -> 3 (c19), 0 (c20), with the remainder now confined to irreducible states (`Math Solver` JS widgets, dynamic `StandardElection` date displays, per-hotel check-in/entity headers) and a one-off tail. New/extended types and rules:
  - **`supercat_cluster`** (new type): the JS-hydrated "supercat" discovery carousel ("What to read" books, "Courses", "Explore stocks", ...). Classified structurally, end-of-chain, by its `Supercat*ClusterTitle` `data-attrid` (reused across content types -- a *books* cluster carries `SupercatRecipeClusterTitle`) rather than the varying heading. The module ships a hidden `display:none` "Something went wrong / TRY AGAIN" async-error fallback whose text (not shown on the page) previously sorted ahead of the real heading and buried it in the component `text`, so it reached unknown; parser emits one row per item (title + author byline; JS-driven cards, no url)
