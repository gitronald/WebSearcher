...
- **Breaking (internal):** rewrote the parse pipeline natively on [selectolax](https://github.com/rushter/selectolax) (lexbor backend), dropping the BeautifulSoup + lxml runtime dependencies. All classifiers, extractors, parsers, and DOM helpers now operate on selectolax nodes; the public `parse_serp` / `SearchEngine` API and core output schema are unchanged (validated against a parser-parity harness over the fixture corpus), but any code reaching into internal node objects is affected — including the exported `make_soup` / `load_soup` helpers, which now return a `selectolax` node instead of a `BeautifulSoup` (bs4-style `.find`/`.select`/`.get_text` calls on their result no longer work)
...
- Optimized `get_text` (the most-called parse helper) with a native-`text()` fast path: when a subtree contains no `script`/`style`/`template` and the separator semantics permit, it delegates to selectolax's C `text()` instead of the pure-Python fragment walker. Byte-identical output (verified over 315k nodes in the fixture corpus; all 87 snapshots unchanged) for ~7% faster `parse_serp` (corpus 3872 -> 3590 ms, median 39.9 -> 36.7 ms/SERP back-to-back, Python 3.13)
- Recovered the body of complementary `kp-wholepage` knowledge panels, which previously emitted near-empty shells (96% of `panel` rows had null text across an entity-crawl corpus). The panel header now yields a clean entity title + descriptor with the tab strip in `details`; the VisualDigest band emits one `featured_results` `sub_rank` row per sub-result (image, web, video, entity facts, social media), keyed by `details.kind` and handling any `VisualDigest*` variant generically; the music-artist sections render one row per section with their items in `details` (songs `{title, album, year}`, albums `{title, year}`, events `{date, time, location, venue}`); `related_searches` splits its accordion dropdown from the grid suggestions (`details.dropdown` vs `details.items`); and the complementary right-hand panel emits one `sub_rank` row per box (Listen / About / Profiles / People also search for) with the box links in `details.{type: hyperlinks, items}`. Validated across two crawls: no SERPs dropped and parse errors fell (8,015 -> 2,792 on a 102k-SERP crawl)
- **Breaking (output):** Renamed the right-hand knowledge-panel rows from `type="knowledge"`/`sub_type="panel_rhs"` to `type="side_bar"`, with `sub_type="panel"` for the main entity panel and `sub_type="links"` for each link box / follow-on section, distinguishing the right-hand sidebar from the main (left) knowledge panel. Consumers filtering on `type="knowledge"`/`sub_type="panel_rhs"` to capture the right-hand panel must switch to `type="side_bar"`
- Dropped the noisy "things to know" Q&A rows from the right-hand sidebar: the expandable per-topic headings (already surfaced cleanly on the main panel row via the panel's `lab/title/*` attrids, with the `iwY1Mb` expander span as a structural fallback) are no longer re-emitted as junk rows, and non-navigational `href="#"` affordance links (general feedback, "Write a review", "Ask a question", social share buttons) are filtered so the boxes they would otherwise fill are skipped — removing duplicate and placeholder rows across entity panels (e.g. gu gels, central park, cistern, file folder, congress, nytimes) while leaving the real link boxes (People also search for, Profiles, Reviews, Events, Neighborhoods) intact
- Fixed a `kp-wholepage` entity-panel bug that dropped the entire organic column when the active tab is one the per-tab recipes do not cover (e.g. a music panel opened to **Albums**): an empty *inactive* sibling-tab shell (the "Songs" tab) claimed the page through the `keep_tokens=None` `standard-songs` recipe — whose under-extract fall-through guard is bypassed for shape-B recipes — and even on the correct sub-column path the active tab's organic column, nested under a single `TzHB6b` marker wrapper, was emitted as one block. A `standard-*` recipe now claims the page only when its detect tab actually carries result-block markers (an empty shell carries none), and `_kp_emit_blocks` descends into a marker wrapper that groups a genuine organic column (two or more titled-link `div.g`) so each organic classifies as `general` — `leandria johnson cd` went from 0 to 10 organics, with the entity/place/shopping panel fixtures (`alexander23`, `central_park`, `gu_gels`) unchanged
