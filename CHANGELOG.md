...
- Renamed the right-hand knowledge-panel rows from `type="knowledge"`/`sub_type="panel_rhs"` to `type="side_bar"`, with `sub_type="panel"` for the main entity panel and `sub_type="links"` for each link box / follow-on section, distinguishing the right-hand sidebar from the main (left) knowledge panel
