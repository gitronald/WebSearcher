...
- Reliable CAPTCHA detection on blocked requests: Google's block redirect to a `/sorry/` page is now detected from the response URL (`utils.is_sorry_redirect`, matching any google-registered host — `www`/ccTLD/`ipv4.google.com`), not just the literal "CAPTCHA" in the page text — `parse_serp` accepts an optional `url` argument that feeds `features["captcha"]`, and `SearchEngine.parse_serp` passes it automatically. The browser backends (`selenium`, `patchright`, `playwright`, `zendriver`) now capture the live URL and any rendered HTML when the `#search` wait times out — but only when the navigation actually moved off the previous page, so a pre-navigation failure can't record the prior query's SERP under the new query — and the `requests` backend records the final post-redirect URL (previously the request URL). A `/sorry/` challenge page is thus saved instead of discarded as an empty response (the patchright/playwright backends also record the real HTTP status, e.g. 429, on timeout). `SearchEngine.parse_serp` additionally resets the previous parse before each run, so a swallowed parse error can no longer leave a stale `captcha` flag attributed to the wrong query. The `ws-demo searches` CAPTCHA guard (wait 5 min, retry once, stop if still blocked) now actually fires on a redirect block instead of looping into it
- Added `update_locations_file` (exported alongside `download_locations`): downloads the latest Google geotargets CSV to one stable path (`data/locations/geotargets.csv`), overwriting it in place, and appends a `date_collected,filename` row to an append-only ledger (`data/locations/ledger.csv`) on each successful pull. Change detection keys on the upstream filename — which embeds the real release date (`geotargets-YYYY-MM-DD.csv`), distinct from the date we happened to collect it — so an unchanged upstream is a no-op. Runnable as `python -m WebSearcher.locations`. The repo now tracks both files (history seeded from a 26-snapshot archive of manual pulls spanning 2018-09-04 through 2026-02-25, one commit per release), and a weekly GitHub Actions cron (`update-locations.yml`) refreshes them and opens a `data`-labeled PR for each new upstream release
- Extracted right-hand-side knowledge-panel entity facts: each `kc:/` fact row now parses to a `side_bar` row with `sub_type="fact"` — the label in `title`, the value in `text` (e.g. "Director" / "David Lean"), and the source attrid plus any links in `details` (`{"type": "hyperlinks", "attrid", "items"}` when linked, `{"type": "item", "attrid"}` otherwise). Previously these rows (director, release date, reviews, address, hours, phone, social profiles, watch providers, ...) were dropped entirely or flattened into untyped box link lists. Box rows no longer re-list links a fact row carries; a box wrapping a single fact lends it its heading (e.g. "Structures", "Watch movie") and is not emitted twice. Link-less section boxes now keep their title and sibling text instead of being dropped, and expander-flagged "Things to know" topics fold into the main panel row even without `lab/title/*` attrids
- Converged the `knowledge` panel `details` shape with the two-tier schema and `knowledge_rhs`: keys are recorded only when informative (no more `heading`/`img_url`/`text` null fills or empty `urls` lists; an empty `details` is `None`), the singular `img_url` became a 1-element `img_urls` list (matching `knowledge_rhs`), and the `dictionary` definition text now lives only in the core `text` field (the `details["text"]` duplicate is gone)
- Enriched video result `details` from the hidden `evlb_*` "About this result" cards modern SERPs pre-render in static HTML: `general[sub_type=video]`, `videos`, `top_stories`/`perspectives`, and `short_videos` rows now carry `details = {"type": "video", title, source, channel, publish_date, thumbnail_url}` (each key only when present). The `short_videos` association is component-scoped so a card can never be borrowed from a neighboring component or an ambiguous wrapper. **Shape note:** video details previously always carried `source` and `duration` keys (None-padded, from selectors stale on modern SERPs); keys are now emitted only when populated, the dead `.gqF9jc` source selector is gone (0 hits corpus-wide), and the legacy `.JIv15d` duration overlay is kept as a merged fallback for older layouts — use `details.get(...)` rather than bracket access
- **Breaking (output):** moved the per-result `error` field off the top level into the `details` extras bucket (two-tier result schema). A parse failure now surfaces as `details = {"type": "item", "error": "<message>"}` instead of a top-level `error` key; `details` always carries a `type` (a specific content label, or the generic `"item"` for a metadata-only row), and stays `None` on clean rows. **Migration:** read `r["details"]["error"]` (guarding `details is None`) instead of `r["error"]`. Also dropped the redundant `general` `"no title or url"` error (equivalent to `title is None and url is None`) and normalized the duplicate `"No subcomponents found"` message to `"no subcomponents parsed"`
- Added a `visible` flag to parsed items: a `details["visible"] = false` is recorded when an item (or its component row) sits under an inline `display:none` lazy-render container (carousel tails / "show more" expansions in the static HTML). Recorded only when hidden — visible items carry no flag. Applies at both the result-row level (`videos`, `top_stories`, `short_videos`, `shopping_ads`, `footer` image cards) and the `details["items"]` level (`top_image_carousel`, `available_on`, `footer` images). As part of this, a non-empty `details` is now guaranteed to carry a `type` (the generic `"item"` when a parser gave content/metadata keys but no specific label), backfilling the previously-typeless `perspectives`/`knowledge`/`locations` payloads
- Recorded item `timestamp` (relative date, e.g. "2 hours ago") in `details["timestamp"]` for `news_quotes`, `twitter_result`, `view_more_news`, and `videos`. These parsers already extracted a timestamp but wrote it to a top-level key the result round-trip silently dropped; it now rides in `details` (only when present, alongside the tweet payload for `twitter_result`). Likewise, `view_more_news` now records its thumbnail `img_url` in `details` (same dropped-top-level-key fix, recorded only when present)
- **Breaking (output):** replaced the `local_results` `sub_type` header-slug derivation with a closed category set — `places`, `locations`, `businesses`, and `availability` (the last from an "in-store availability" header). The old `slugify(header)` fallback minted roughly one junk per-query `sub_type` from query-/location-specific header text (e.g. `river_forest,_il`, `these_are_results_for__amour_de_hair_nyc…`); a header outside the closed set now carries **no** `sub_type` (the raw header is still preserved in `details["heading"]`). Consumers filtering on the former `results_for`/slug values must switch
- Made `buying_guide` and `products` classification robust to non-English or reworded headings by adding a structural CSS-class dispatch path (e.g. the `buying_guide` `ITWcLb` signal) ahead of the header-text match; classifications observed on the fixture corpus are unchanged
...
- **Breaking (internal):** renamed the `WebSearcher.search_methods` subpackage to `WebSearcher.searchers` and moved the `SearchEngine` module into it. The public entrypoints are unchanged — `import WebSearcher; WebSearcher.SearchEngine` and `from WebSearcher.searchers import SearchEngine` both still work — but direct imports of the browser backends must switch from `WebSearcher.search_methods.<backend>_searcher` to `WebSearcher.searchers.<backend>_searcher`
