...
Implementation plans for project features and improvements. Each plan ([`plans/`](plans/)) has YAML frontmatter tracking status, branch, and timestamps, and is linked from `TODO.md` in the project root — open tasks reference their plan file, completed tasks retain the link as a historical record.
